...
(define-fun-rec s2tree ((s String) (i Int) (acc Tree)) Tree (
	ite (< i 0) acc
		(s2tree s (- i 1) (insert acc (ij2v (s2cl (str.at s i)) (+ i 1))))
))

(define-fun flag2s ((s String) (t String)) Bool
	(exists ((x Tree))
		(and
			(= x (s2tree s (- (str.len s) 1) leaf))
			(tostr x t)))
)
